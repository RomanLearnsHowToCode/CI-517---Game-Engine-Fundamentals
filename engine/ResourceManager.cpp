...
SDL_Texture * ResourceManager::loadTexture(std::string file, SDL_Color trans) {
	...
	*/
	...
	/*
		Probably the hardest feature I had to impelement, the getTexture wasn't working well, map was returning NULL!
	*/

	if (texture != nullptr) {
		
		std::map<std::string, SDL_Texture *>::iterator iterator;
		iterator = textures.find(file);
...
SDL_Texture * ResourceManager::getTexture(std::string fileName) {
	...
	//SDL_Texture* result = textures[fileName]; fixed
